...
|> String.split("\n")
|> Enum.map(fn(line) ->
  system = "gameboy_color"
  case String.split(line, ",") do
    [name_part, sub_part, pub, date, _system] ->
      real_name = String.trim_trailing(sub_part, "\"")
        <> " "
        <> String.trim_leading(name_part, "\"")
        |> String.trim()
    ...
    [name, pub, date, _system] ->
      real_date = parse_date.(date)
      %Gamelists.Games.Game{name: String.trim(name), publisher: pub, release_date: real_date, system: system}
      |> Gamelists.Repo.insert!
    [""] -> nil
  end
end)
